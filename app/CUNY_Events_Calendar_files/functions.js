jQuery(function($){
    ...
    $('.close-sliding-nav').click(function(e){
        ...
        $('.menu-toggle').siblings('i').each(function(){
            $(this).addClass('icon-swipe_d').removeClass('icon-swipe_u');
        });
    });
    ...
    enquire.register("screen and (max-width:779px)", {
        match: function(){
            ...
            if ( nav.length == 1 ) {
                ...
                nav.detach().addClass('hidden').appendTo('#mobile-contextual-menu-container');
                // attr('id', 'contextual-nav-content')
            }
            else {
                $( '#contextual-menu-toggle' ).hide();
            }

            // Attach the side navigation after the main content
            $('#wrap-content > .vc_row:first > .vc_col-sm-3:first').remove().appendTo($('#wrap-content > .vc_row:first'));
        },   
        ...
        unmatch: function(){
            ...
            // Contextual Menu
            // nav = $('ul.menu, ul#menu-side-navigation');
            // if (nav.length == 1){
            ...
            //$('.contextual-nav-parent-placeholder').remove();

            $('#wrap-content > .vc_row:first > .vc_col-sm-3:last').remove().prependTo($('#wrap-content > .vc_row:first'));
        }
    });
